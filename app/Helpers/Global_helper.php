...
function profile_image_super(){

    $userIdAdmin = newSession()->userIdAdmin;

    $table = DB()->table('admin');
    $query = $table->where('user_id',$userIdAdmin)->get()->getRow();
...
function globalStatus($sele = 'sel')
{
    $status = [
        '1' => 'Active',
        '0' => 'Inactive',
...
function shopCateListInOption($categoryId)
{
    ...
    $query = $table->where('parent_cat_id', '0')->get();
...
function shopSubCatListInOption($categoryId)
{
    ...
    $table = DB()->table('shop_category');
    $query = $table->where('parent_cat_id', $catId)->get();
...
function priorityType($select)
{
    ...
    foreach ($status as $key => $value) {
        $options .= '<option value="' . $key . '" ';
        $options .= ($key == $select) ? ' selected="selected"' : '';
        $options .= '>' . $value . '</option>';
    }
    return $options;
}

function home_feature($select)
{
    $status = [
        '0' => 'Inactive',
        '1' => 'Active',
...
function shopSubCatListOption($sel='')
{
    ...
    foreach ($query->getResult() as $value) {
        $options .= '<option value="' . $value->shop_cat_id . '" ';
        $options .= ($value->shop_cat_id == $sel) ? ' selected="selected"' : '';
        $options .= '>' . $value->name . '</option>';
    }
    return $options;
}
...
{
    $table = DB()->table($table);
    $query = $table->where($colam,$value)->where('deleted IS NULL')->countAllResults();
...
function getinvoiceStatusNew($id){
    $table = DB()->table('invoice');
    $query = $table->where("invoice_id", $id)->get()->getRow();


    if ($query->status == 0){
        $result = '<span class="label bg-primary p-1">Unpaid</span>';
    }
    if ($query->status == 1) {
        $result = '<span class="label bg-success p-1">Paid</span>';
    }
    if ($query->status == 2){
        $result = '<span class="label bg-warning p-1">Pending</span>';
    }
    if ($query->status == 3){
        $result = '<span class="label bg-danger p-1">Cancel</span>';
    }
...
function deliverystatus($invoiceId)
{
    ...
    $query =$table->where('invoice_id', $invoiceId)->get();
    return $query->getResult();
}

function package_id_by_product($packageId)
{
    $table = DB()->table('invoice_item');
    $query = $table->where('package_id', $packageId)->get()->getResult();
    return $query;
}
...
function invoice_id_by_package($invoiceId)
{
    ...
    $query = $table->where('invoice_id', $invoiceId)->get()->getResult();
    return $query;
}

function deliverystatus3($packageId)
{
    $table = DB()->table('delivery');
    ...
    if (!empty($query)) {
        $status = $query->status;
        if ($status == 0) {
            $data = '<span class="label label-info">Processing</span>';
        }
        if ($status == 1) {
            $data = '<span class="label label-success">Complete</span>';
        }

    } else {
        $data = '<span class="label label-warning">Place order</span>';
    }
    return $data;
}
...
function getPackageStatus($id)
{
    ...
    $table = DB()->table('package');
    $query = $table->where("package_id", $id)->get()->getRow();
    if ($query->status == '1') {
        $mess = '<span class="label label-success">Paid</span>';
    }
    if ($query->status == '0') {
        $mess = '<span class="label label-success">Unpaid</span>';
    }
    if ($query->status == '2'){
        $mess = '<span class="label label-danger">Cancel</span>';
    }
    return $mess;
...
function global_address_id_by_address($id)
{
    ...
    $query = $table->where("global_address_id", $id)->get()->getRow();
    if (!empty($query)) {
        $view = $query;
    } else {
        $view = "0";
...
function get_due_commision_seller($sch_id, $userId)
{
    ...
    $duecom = $table->selectSum('commission')->join('package', 'commission.invoice_id = package.invoice_id')->where('package.sch_id =' . $sch_id)->where('commission.seller_id = ' . $userId)->where('commission.com_status', '0')->get()->getRow()->commission;
    return $duecom;
}
...
{
    $table = DB()->table('commission');
    $reccom = $table->selectSum('commission')->join('package', 'commission.invoice_id = package.invoice_id')->where('package.sch_id =' . $sch_id)->where('commission.seller_id = ' . $userId)->where('commission.com_status', '1')->get()->getRow()->commission;
    return $reccom;
}
...
function get_due_commision_delivery_boy($sch_id, $deliveryBoyId)
{
    ...
    $duecom = $table->selectSum('commission')->join('package', 'commission.package_id = package.package_id')->where('package.sch_id =' . $sch_id)->where('commission.delivery_boy_id = ' . $userId)->where('commission.com_status', '0')->get()->getRow()->commission;
    return $duecom;
}
...
function get_received_commision_delivery_boy($sch_id, $deliveryBoyId)
{
    ...
    $reccom = $table->selectSum('commission')->join('package', 'commission.package_id = package.package_id')->where('package.sch_id =' . $sch_id)->where('commission.delivery_boy_id = ' . $userId)->where('commission.com_status', '1')->get()->getRow()->commission;
    return $reccom;
}

function shop_id_by_total_sale($shopId){
    $table = DB()->table('sup_commi_invoice');
    $result = $table->selectSum('amount')->where('sch_id', $shopId)->get()->getRow()->amount;
    return $result;
}

function shop_id_by_total_sale_commission($shopId){
    $table = DB()->table('sup_commi_invoice');
